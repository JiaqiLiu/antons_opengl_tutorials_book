...
int main () {
  ...
  while (!glfwWindowShouldClose (g_window)) {
    ...
    if (glfwGetKey (g_window, GLFW_KEY_LEFT)) {
      matrix[12] -= speed * elapsed_seconds;
      moved = true;
    }
    ...
    if (glfwGetKey (g_window, GLFW_KEY_UP)) {
      matrix[13] += speed * elapsed_seconds;
      moved = true;
    }
    ...
    // Jiaqi: This seems not work, because we don't have perspective projection. 
    if (glfwGetKey (g_window, GLFW_KEY_0)) {
      matrix[14] += speed * elapsed_seconds;
      moved = true;
    }
